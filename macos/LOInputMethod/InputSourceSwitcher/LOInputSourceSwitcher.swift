...
///   `TISSelectInputSource` 刚切回语镜，系统原生 CapsLock 处理立刻又切走。
...
class LOInputSourceSwitcher {
    ...
    func start() {
        ...
        guard let tap = CGEvent.tapCreate(
            ...
            debugLog("CGEventTap 创建失败，可能缺少辅助功能权限。请在系统设置→隐私与安全性→辅助功能中授权语镜。")
    ...
    private func switchToLo() {
        ...
        if let source = cachedSelfInputSource {
            ...
            let status = TISSelectInputSource(source)
            isInABCMode = false
            debugLog("切回语镜(缓存): status=\(status)")
            return
        }

        // 缓存为空：尝试用 TISCopyCurrentKeyboardInputSource 找语镜
        // （此时当前是 ABC，这个方法返回 ABC 不是语镜，无法用）
        debugLog("切回语镜失败：无缓存源，请用 Ctrl+Space 手动切回")
    }
